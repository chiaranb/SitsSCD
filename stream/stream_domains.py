...
# ---------------- Configuration ----------------
wandb.login()
PROJECT_NAME = "capymoa-domain_shift" 
...
SCENARIOS_TO_RUN = ["spatial", "temporal_spatial"]
...
def run_scenario_experiment(
    ...
    """
    Esegue un esperimento completo (train + prequential) per un dato scenario
    e un set di embedding.
    """
    print(f"--- Avvio Scenario: {scenario_name} ---")
    ...
    # Dati di Training (Sempre 2018 @ Siti A)
    df_train = df[
        (df['timestamp'] < 365) & 
        (df['sits_id'].isin(train_ids))
    ].reset_index(drop=True)

    # Dati di Prequential (dipende dallo scenario)
    ...
    for model_name, model_class in tqdm(MODELS.items(), desc=f"Models ({scenario_name})", leave=False):
        ...
        try:
            ...
            # 4️⃣ Train iniziale (2018 @ Siti A)
            ...
            # 6️⃣ Final metrics
            final_metrics = {
                "scenario": scenario_name, # <-- Colonna aggiunta
...
# Carica il file JSON degli split UNA VOLTA
try:
    with open(SPLITS_JSON_PATH) as f:
        sits_splits = json.load(f)
    print(f"Caricato {SPLITS_JSON_PATH}")
except FileNotFoundError:
    print(f"ERRORE: File split non trovato: {SPLITS_JSON_PATH}")
...
if all_results_in_memory:
    ...
    # Ordina per scenario e accuratezza per una tabella più chiara
